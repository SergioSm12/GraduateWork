...
export const ShowUserVehicles = ({id, userSelected }) => {
  ...
  return (
    <div className="flex flex-col bg-secondary-900 p-4 rounded-lg">
      <div className="flex justify-end mb-3">
        <button className="flex items-center gap-2 font-bold text-xs py-2 px-4 bg-primary/80 text-black hover:bg-primary rounded-lg transition-colors ">Agregar Vehiculo <RiRoadsterLine/></button>
      </div>
      {userShow.vehicles.length > 0 ? (
        <DataTableVehicle id={id}/>
      ) : (
