import React, { useEffect } from "react";
import { useReports } from "../../../hooks/useReports";
import { Link } from "react-router-dom";
import { LinesChart } from "../../Statistics/LinesChart";
import { GenerateReportMonthlySpecific } from "../ReceiptReports/GenerateReportMonthlySpecific";
import { DataTableReports } from "../DataTableReports";

export const UnifiedReport = () => {
  const {
    currentMonthlyUnifiedReceiptReport,
    currentWeeklyUnifiedReceiptReport,
    currentBiweeklyUnifiedReceiptReport,
    getCurrentMonthlyUnifiedReceiptReport,
    getCurrentBiweeklyUnifiedReceiptReport,
    getCurrentWeeklyUnifiedReceiptReport,
  } = useReports();

  useEffect(() => {
    getCurrentMonthlyUnifiedReceiptReport();
    getCurrentBiweeklyUnifiedReceiptReport();
    getCurrentWeeklyUnifiedReceiptReport();
  }, []);

  return (
    <>
     <div className="mb-10">
        <h1 className="font-bold text-gray-100 text-xl">Reporte de ingresos</h1>
        <div className="flex items-center gap-2 text-sm text-gray-500">
          <Link to="/" className="hover:text-primary transition-colors">
            Principal
          </Link>
          <span>-</span>
          <span>Centro de reportes recibos "diurnos, nocturnos y visitantes"</span>
        </div>
      </div>
      <div className="grid grid-cols-1">
        <div className="flex justify-center bg-secondary-100 p-8 rounded-lg overflow-auto">
          <div className="w-[450px] h-[230px] md:w-full md:h-full">
            <LinesChart dataReport={currentMonthlyUnifiedReceiptReport} />
          </div>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
        <div className="bg-secondary-100 p-8 rounded-lg">
          <GenerateReportMonthlySpecific reportType={"unified"} />
        </div>
        <div className="bg-secondary-100 p-8 rounded-lg">
          <DataTableReports dataReports={currentMonthlyUnifiedReceiptReport} />
        </div>
        <div className="bg-secondary-100 p-8 rounded-lg">
          <DataTableReports dataReports={currentBiweeklyUnifiedReceiptReport} />
        </div>
        <div className="bg-secondary-100 p-8 rounded-lg">
          <DataTableReports dataReports={currentWeeklyUnifiedReceiptReport} />
        </div>
      </div>
    </>
  )
};
